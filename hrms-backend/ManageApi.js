...
const handleErr = (res, err, ctx) => {
  ...
  error: "This email is already registered!" 
    ...
    if (err.message.includes('employee_id'))
      return res.status(400).json({ success: false, error: "This Employee ID already exists!" });
...
app.post('/api/employees/add', async (req, res) => {
  ...
  try {
    ...
    const [existingId] = await db.promise().query(
      "SELECT id FROM employees WHERE employee_code = ?", [d.employee_code]
    );
...
return res.status(400).json({ 
  ...
  error: "This Employee ID already exists!" 
    ...
    const [existingEmail] = await db.promise().query(
      "SELECT id FROM employees WHERE email = ?", [d.email]
    );
    ...
    await db.promise().query(sql, [
      d.employee_code, first_name, last_name,
      d.dept || null,
      department_id,
      d.designation || null, d.email || null,
      d.job_role || 'employee', d.status || 'Active',
      hash, d.current_salary || 0,
      d.joining_date || null, d.reporting_manager || null,
      d.phone_no || null,
      d.kpi || null,  // ✅ নতুন
    ]);
    res.json({ success: true, message: "Employee added successfully!" });

  } catch (e) { handleErr(res, e, 'ADD_EMP'); }
});
...
app.post('/api/employees/update', async (req, res) => {
  ...
  try {
    ...
    const [r] = await db.promise().query(sql, [
      ...
      d.designation || null, d.email || null,
      d.job_role || 'employee', d.status || 'Active',
      d.current_salary || 0, d.joining_date || null,
      d.reporting_manager || null, d.phone_no || null,
      d.kpi || null,        // ✅ নতুন
      d.employee_code,
    ]);
  ...
  } catch (e) { handleErr(res, e, 'UPDATE_EMP'); }
});
// ── DELETE Employee ──────────────────────────
app.post('/api/employees/delete', async (req, res) => {
  ...
  } catch (e) { handleErr(res, e, 'DELETE_EMP'); }
});
...
app.get('/api/salary/:employee_code', async (req, res) => {
  ...
  try {
    ...
    const basic             = parseFloat((gross_salary * 0.50).toFixed(2));
    const hra               = parseFloat((gross_salary * 0.20).toFixed(2));
    const conveyance        = 2000;
    const medical           = 1500;
    const special_allowance = Math.max(parseFloat((gross_salary - basic - hra - conveyance - medical).toFixed(2)), 0);
    const gross             = parseFloat((basic + hra + conveyance + medical + special_allowance).toFixed(2));
    const pf                = parseFloat((basic * 0.12).toFixed(2));
    const esi               = gross_salary <= 21000 ? parseFloat((gross * 0.0075).toFixed(2)) : 0;
    const pt                = 200;
    const tds               = parseFloat((gross * 0.05).toFixed(2));
    const other             = 500;
    const total_deductions  = parseFloat((pf + esi + pt + tds + other).toFixed(2));
    const net_salary        = parseFloat((gross - total_deductions).toFixed(2));

    res.json({
      success: true,
      employee: { name: emp[0].name, designation: emp[0].designation, employee_code },
      salary: {
  ...
  } catch (e) { handleErr(res, e, 'GET_SALARY'); }
});
